...
--------------------
    ...
    PROCEDURE copy_table(p_source_scheme IN VARCHAR2,
      ...
      BEGIN
          LOG_UTILS.log_start('copy_table', 'Копіювання з ' || p_source_scheme || ' до ' || p_target_scheme);
          ...
          FOR rec IN (
              ...
              FROM (
                  ...
                  WHERE owner = UPPER(p_source_scheme)
                    AND table_name IN (SELECT value_list FROM TABLE(UTIL.table_from_list(UPPER(p_list_table))))
              )
              GROUP BY table_name
          ) LOOP
              BEGIN
                  -- Створення структури таблиці
                  EXECUTE IMMEDIATE rec.ddl_code;
                  LOG_UTILS.log_finish('copy_table', 'Створено таблицю: ' || rec.table_name);

                  -- Копіювання даних (якщо p_copy_data = TRUE)
                  IF p_copy_data THEN
                      v_sql_insert := 'INSERT INTO ' || p_target_scheme || '.' || rec.table_name || 
                                      ' SELECT * FROM ' || p_source_scheme || '.' || rec.table_name;
                      EXECUTE IMMEDIATE v_sql_insert;
                      v_count_tables := v_count_tables + SQL%ROWCOUNT;
                      LOG_UTILS.log_finish('copy_table', 'Дані скопійовано в: ' || rec.table_name);
                  END IF;

              EXCEPTION
                  WHEN OTHERS THEN
                      LOG_UTILS.log_error('copy_table', 'Помилка копіювання ' || rec.table_name || ': ' || SQLERRM);
                      CONTINUE; 
              END;
          ...
          po_result := 'Оброблено об''єктів: ' || v_count_tables;
          LOG_UTILS.log_finish('copy_table', po_result);
